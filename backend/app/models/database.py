...
def _load_local_db() -> dict:
    if not DB_FILE.exists():
        with open(DB_FILE, "w", encoding="utf-8") as f:
            json.dump({"embeddings_marca": [], "posts_salvos": []}, f)
        return {"embeddings_marca": [], "posts_salvos": []}
    try:
        with open(DB_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
            if "posts_salvos" not in data:
                data["posts_salvos"] = []
            return data
    except Exception:
        return {"embeddings_marca": [], "posts_salvos": []}
...
async def salvar_post_supabase(
    cliente_id: str,
    titulo_interno: str,
    ...
    produto_url: str | None = None,
    imagem_url: str | None = None,
    imagens_carrossel_urls: list[str] | None = None,
    ...
    new_post = {
        ...
        "sugestao_de_edicao_visual": sugestao_de_edicao_visual,
        "hashtags": hashtags,
        "local_do_preco": local_do_preco,
        "produto_id": produto_id,
        "produto_url": produto_url,
        "imagem_url": imagem_url,
        "imagens_carrossel_urls": imagens_carrossel_urls or [],
        "imagem_disponivel": imagem_disponivel,
        "created_at": datetime.datetime.now(datetime.timezone.utc).isoformat()
    }
    db["posts_salvos"].append(new_post)
    _save_local_db(db)
    return new_post


async def listar_posts_supabase(cliente_id: str) -> list[dict]:
    db = _load_local_db()
    posts = [
        p for p in db.get("posts_salvos", [])
...
async def deletar_post_supabase(post_id: str) -> bool:
    db = _load_local_db()
    initial_len = len(db.get("posts_salvos", []))
    
    # Remove também qualquer imagem física local gerada para o post deletado
    for p in db.get("posts_salvos", []):
        if p.get("id") == post_id:
            for img_url in [p.get("imagem_url")] + (p.get("imagens_carrossel_urls") or []):
    ...
    if len(db["posts_salvos"]) < initial_len:
        _save_local_db(db)
        return True
    return False
