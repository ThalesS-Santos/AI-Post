...
class GeneratePostRequest(BaseModel):
    cliente_id: UUID
    foco_semana: Optional[str] = Field(default="", max_length=500)
...
class SalvarPostRequest(PostContent):
    cliente_id: UUID


class PostSalvoResponse(BaseModel):
    id: UUID
    cliente_id: UUID
    titulo_interno: str
    legenda_instagram: str
    ...
    imagens_carrossel_urls: List[str] = Field(default_factory=list)
    imagem_disponivel: bool
    created_at: datetime.datetime


class ListarPostsResponse(BaseModel):
    status: str
    posts: List[PostSalvoResponse]
